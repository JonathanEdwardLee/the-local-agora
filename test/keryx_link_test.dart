...
void main() {
  ...
  testWidgets('WHEN/WHAT changes do not probe Firebase', (tester) async {
    ...
    await tester.tap(find.byKey(const ValueKey('jf-when-tomorrow')));
    await tester.pump(const Duration(milliseconds: 50));
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('jf-what-comedy')));
    await tester.pump(const Duration(milliseconds: 50));
    await tester.pump();
    expect(fake.probeCount, 0);
  });
  ...
  testWidgets('main Scan button does not call link service', (tester) async {
    ...
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('jf-when-nextSevenDays')));
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('jf-what-music')));
    await tester.pump();
