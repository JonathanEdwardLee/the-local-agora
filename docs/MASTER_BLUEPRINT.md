...
# 22\. POST-CONTEST SHARED PLATFORM ROADMAP


## One event system, multiple front ends


The Local Agora is one shared event-data platform served through multiple clients:


- Android application
- Public Flutter web machine
- Future native iPhone application from the same Flutter codebase
- One protected Firebase/Keryx backend
- One shared collection of sourced, normalized event records


Android, web, and future iOS clients must query the same protected service layer and shared event index. They must not create separate incompatible databases or product logic.


The public web machine should launch alongside or shortly after the Android public release. It gives iPhone, desktop, tablet, and installation-averse visitors immediate access while native iOS demand is measured.


Preferred public structure:


- `junkfeathers.com/local-agora/` — search-friendly WordPress landing, explanation, support, privacy, Android promotion, iPhone demand capture, and Junkfeathers discovery
- `agora.junkfeathers.com` — preferred full-screen Flutter web machine, subject to final hosting and DNS verification


A subpath may be used if it is materially easier to maintain, but the marketing site and the Flutter machine should remain separate systems with a clear connection.


## Cache-first Keryx behavior


The backend should treat stored current event records as the first source for ordinary user requests.


Recommended request sequence:


1. Normalize the requested city, ZIP/postal code, time window, and category.
2. Query the shared event index for current matching records.
3. Evaluate cache freshness, coverage, last scan, source health, and refresh eligibility.
4. Return useful fresh records immediately when available.
5. Invoke paid discovery and normalization only when records are missing, stale, incomplete, or an approved rescan is justified.
6. Merge newly supported findings into the shared index through server-side validation and deduplication.
7. Serve the improved index to later Android and web visitors.


Do not permanently cache every exact filter combination as an isolated product. Store reusable event facts and source records, then filter those facts by location, date, and category.


The largest expected cost benefit is avoiding repeated AI/search work. Storage cleanup is still useful, but it is not a substitute for rate limits, freshness rules, abuse protection, and server-side refresh decisions.


## Event lifecycle and expiration


Active public results never include events whose end time has passed, except when a clearly marked correction or administrative review needs them.


Recommended lifecycle:


1. `UPCOMING` — active and eligible for public discovery.
2. `ACTIVE` — currently occurring where an end time is known.
3. `RECENTLY_EXPIRED` — hidden from normal public results but retained temporarily for corrections, duplicate detection, source review, and abuse investigation.
4. `PURGED` — the full event record is removed after the retention window.


Initial retention target:


- Keep recently expired full records for approximately 14–30 days.
- Use a configurable server policy rather than hardcoding one permanent interval.
- After full-record deletion, retain only a minimal non-public fingerprint when justified for duplicate prevention, source history, moderation, or abuse controls.


A minimal retained fingerprint may include:


- hashed or normalized source identifier
- venue identity where public
- event-title fingerprint
- event date
- deletion or expiration reason


Do not retain hidden private addresses, unnecessary personal information, flyer images, or bulky obsolete fields merely for history.


Firestore TTL or a scheduled server cleanup may implement expiration after the data model and retention policy are physically tested. Cleanup must be observable, reversible during beta where practical, and included in cost monitoring.


## Public website and launch-before-Pro rule


The useful free discovery loop, shared cache, Android client, public web machine, support/privacy pages, and cost controls should launch before visible Pro accounts.


Recommended order:


1. Prove sourced Keryx scans.
2. Store and reuse normalized current event records.
3. Add deduplication, freshness, expiration, and abuse controls.
4. Release Android publicly.
5. Release the public Flutter web machine.
6. Add the Local Agora landing page and iPhone demand capture to Junkfeathers.com.
7. Measure usage, cache-hit rate, successful-event cost, source quality, and city demand.
8. Add accounts and Pro only when the free system proves value and users demonstrate a specific paid need.


Pro-account work must not delay a useful public free release.


## Website cross-promotion boundary


The Local Agora landing page may promote:


- the Android application
- the public web machine
- future iPhone availability
- Orpheus Deck
- Junkfeathers music
- other relevant Junkfeathers Tech projects


Do not place promotional interruptions inside chronological event results or between records. Cross-promotion belongs on the landing page, About, Settings, footer, or a deliberately opened “More from Junkfeathers” surface.


## Measuring native iPhone demand


The public web machine is the immediate iPhone-access path. Native iOS distribution is an evidence-based expansion rather than a prerequisite for public launch.


The website may collect:


- a deduplicated anonymous `REQUEST IOS VERSION` count
- an optional verified email waitlist for a one-time iPhone launch notice
- aggregate web-device usage showing how many Local Agora sessions come from iPhones


Approved privacy language:


> Enter your email to receive one notice when the iPhone version becomes available. We will not add you to unrelated marketing.


A general Junkfeathers newsletter must use separate, optional consent and must not be preselected.


Begin the native iPhone release when at least two of these conditions are true:


1. At least 100 verified iPhone waitlist emails.
2. At least 25% of Local Agora web usage comes from iPhones for two consecutive months.
3. Junkfeathers Tech has at least $300 in available business cash for Apple fees, testing, and incidental release costs.
4. Jonathan has reliable access to a Mac and at least one physical iPhone for testing.


Do not display fake urgency, a manipulated progress counter, or an unverifiable “votes remaining” claim.


## Post-launch evidence dashboard


Before Pro or native iOS, review at least:


- searches by city and region
- cache-hit rate
- rescan rate
- AI/search cost per successful new event
- duplicate rate
- expired-event cleanup volume
- source success and correction rate
- Android versus web usage
- iPhone share of web traffic
- verified iOS waitlist size
- public event-record read/write volume


Collect only what is needed for product, safety, and cost decisions. Do not introduce invasive tracking merely because analytics are available.




# 23\. FINAL COUNCIL DIRECTIVE
